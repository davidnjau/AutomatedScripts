...
- **`dlv_core.py`** — the DLV queue storage (`load_dlv_batch`/`save_dlv_batch`/etc.) and the assessor/DLV search-and-classify layer shared by DLV Batch and DLV Tasks. Also owns `DLV_TAGS`, the fixed tag vocabulary (Queue/Direct) DLV Batch's optional "🏷 Tag Tasks" step assigns per ref and DLV Tasks' "By Tag" report filters on — a queue item's `tag` field rides through to the closed store automatically since closed records are built by spreading the item dict.
