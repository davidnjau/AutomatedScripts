...
- **`dlv_core.py`** — the DLV queue storage (`load_dlv_batch`/`save_dlv_batch`/etc.) and the assessor/DLV search-and-classify layer shared by DLV Batch and DLV Tasks. Also owns `DLV_TAGS`, the fixed tag vocabulary (Queue/Direct) DLV Batch's optional "🏷 Tag Tasks" step assigns per ref and DLV Tasks' "By Tag" report filters on — a queue item's `tag` field rides through to the closed store automatically since closed records are built by spreading the item dict. `dlv_incremental.py`'s auto-sequenced "B{batch}-T{task}" tag is a third Tag Tasks option but lives in its own module, not here, since it's unique-per-ref rather than a small filterable vocabulary.
...
- `saved_incremental_counter.json` — `{batch_number, task_number}`, the current position `dlv_incremental.py`'s `next_incremental_tag()` consumes from and advances (wraps to the next batch after task 6); seeded via ⚙️ Set Counter, not any built-in default beyond batch 1 task 1
