...
- **`dlv_core.py`** — the DLV queue storage (`load_dlv_batch`/`save_dlv_batch`/etc.) and the assessor/DLV search-and-classify layer shared by DLV Batch and DLV Tasks. Also owns `DLV_TAGS`, the fixed tag vocabulary (Queue/Direct) DLV Batch's optional "🏷 Tag Tasks" step assigns per ref and DLV Tasks' "By Tag" report filters on — a queue item's `tag` field rides through to the closed store automatically since closed records are built by spreading the item dict. Also owns `INCREMENTAL_TAG_SENTINEL`/`parse_incremental_tag`/`is_incremental_tag` for `dlv_incremental.py`'s auto-sequenced "B{batch}-T{task}" tag — a third Tag Tasks option that's unique-per-ref rather than a small filterable vocabulary, so it lives here (not in `dlv_incremental.py` or `dlv_tasks.py`, both of which need it and the former already imports from the latter) rather than either feature module. `dlv_tasks.py`'s "By Tag" picker has its own "🔢 Incremental" entry using the same sentinel as a filter meaning "every incremental-tagged ref together" (`_dt_tag_matches`), distinct from `dlv_batch.py`'s use of the same sentinel to mean "generate a new incremental tag for this ref". **Owns the consolidated ref-keyed store** (`saved_dlv_records.json`, `_load_consolidated`/`_save_consolidated`) that backs `saved_dlv_batch.json`/`saved_dlv_closed.json` *and* `common.py`'s `saved_assignments.json` *and* `hold_tasks.py`'s `saved_hold_tasks.json` — a JSON-file consolidation replacing "which file is this ref in" with a `status` field (`queued`/`assigned`/`completed`/`returned`/`removed`) on one record per ref, so a ref's queue/assignment/hold/closed history is never split across files that have to be separately loaded and stitched back together. `load_dlv_batch`/`save_dlv_batch`/`load_dlv_closed`/`_append_dlv_closed` (here), `load_saved_assignments`/`persist_assignment` (`common.py`), and `load_hold_tasks`/`save_hold_tasks` (`hold_tasks.py`) all keep their original signatures/return shapes as adapters over this one store, so no other call site in the bot changed. `mark_removed`/`clear_hold_and_remove` mark a ref `status="removed"` (with `removed_at`) rather than deleting it outright when it's bulk-deleted from the queue or released from hold — nothing is ever silently erased. Every write merges onto the existing record (`{**existing, **new_fields}`), never replaces wholesale, so a field learned at one stage (e.g. `parcel`/`tag` while queued) survives into later stages (assigned, held, closed) — this fixed a real bug in the old `persist_assignment`, which used to overwrite a ref's record wholesale, silently dropping enrichment a richer caller had added if a later caller passed no `extra`. The four legacy files (`saved_dlv_batch.json`, `saved_dlv_closed.json`, `saved_assignments.json`, `saved_hold_tasks.json`) are migrated into the consolidated store once, on first read (`_migrate_legacy_stores`, precedence: assignments as the base layer, then queued, then closed, then hold attached last), and left on disk untouched, per this codebase's existing migration convention. `common.py`'s `load_saved_assignments`/`persist_assignment` import from here locally (inside the function body, not at module level) since `dlv_core.py` already imports from `common.py` at module level and a module-level import back would be circular; `hold_tasks.py` imports at module level since that direction already existed with no cycle.
...
Menu buttons (all `BTN_*` constants defined in `common.py`, laid out by `_main_menu()`): `📋 New Assignment`, `📊 Fetch Tasks`, `⏰ Auto Fetch`, `🗂 AF Results`, `📜 Assignments`, `📥 DLV Batch`, `📤 Export Valuation Report`, `📊 Export Status`, `🏆 Job Distribution`, `👤 Valuer Tasks`, `📋 DLV Tasks`, `📧 DLV Report Schedule`, `🔲 Sectional`, `🌅 Morning Briefing`, `✋ Hold Tasks`, `🔢 Incremental`, `🔎 Lookup Reference`, `🔑 Refresh Auth`, `🔒 Token Status`, `📉 Error Report`, `👥 Saved Valuers`, `🗑 Delete Valuer`, `🔄 Token Daemon`, `🔁 Restart Bot`, `❓ Help`, `🛑 Cancel`. Receive Tasks (`/receive`) and DLV Queue (`🔍 DLV Queue`, reached from within the DLV Batch flow) have no top-level menu entry; Hold Tasks' own "View Held Queue" sub-view is reached from within `✋ Hold Tasks` the same way.
...
- `saved_assignments.json` — `{ref_number: {valuer_name, valuer_uid, assigned_at}}` — assignment history (used to detect reassignments). **Legacy file** — `load_saved_assignments`/`persist_assignment` are now adapters over `saved_dlv_records.json` (see below); this file is read once, at migration time, and then left on disk untouched, no longer written to
...
- `saved_hold_tasks.json` — `[{ref, held_valuer_name, held_valuer_uid, held_at, last_checked, last_error}]` — Hold Tasks' guarded-refs queue. **Legacy file**, same as `saved_assignments.json` above — `load_hold_tasks`/`save_hold_tasks` are now adapters over `saved_dlv_records.json`'s `hold` sub-object
- `saved_dlv_batch.json` / `saved_dlv_closed.json` — the DLV Batch active queue and closed store (`[{ref, valuer_name, valuer_uid, valuer_acct, queued_at, assessor, tag, last_error}]`); `tag` is optional, set via DLV Batch's "🏷 Tag Tasks" step from `dlv_core.DLV_TAGS` or `dlv_incremental.py`'s auto-sequenced value, and carries through to the closed record automatically. **Legacy files**, same as above — `load_dlv_batch`/`save_dlv_batch`/`load_dlv_closed`/`_append_dlv_closed` are adapters over `saved_dlv_records.json`
- `saved_dlv_records.json` — `{ref: {status, valuer_name, valuer_uid, valuer_acct, tag, assessor, parcel, consideration, currency_code, queued_at, assigned_at, closed_reason, application_status, node, request_type, consideration_amount, closed_at, last_error, hold, removed_at}}` — the one consolidated ref-keyed DLV-lifecycle store (`dlv_core._load_consolidated`/`_save_consolidated`) that `saved_dlv_batch.json`/`saved_dlv_closed.json`/`saved_assignments.json`/`saved_hold_tasks.json` above are now legacy views onto — `status` (`queued`/`assigned`/`completed`/`returned`/`removed`) replaces "which file is this ref in," and `hold` (a sub-object, `None` when not held) replaces "which held-queue entry." No count-based cap on size — the old `saved_assignments.json` 500-entry rotation was dropped, matching `saved_dlv_closed.json`'s always-unbounded retention
- `saved_dlv_report_schedules.json` — `[{id, scope, target_key, target_name, period_days, period_label, interval_minutes, email}]`, every active 📧 DLV Report Schedule (`dlv_report_schedule.py`) — a recurring emailed version of DLV Tasks' own By Valuer/By Tag report, sourced from the same local JSON stores via `dlv_tasks.py`'s `_dt_gather_report_data`/`_dt_collect_valuers` (no live API calls); each schedule gets its own repeating job (`drs_job:{id}`), mirroring Auto Fetch's per-schedule job pattern, and is emailed as an Excel workbook via `email_service._send_bulk_export_email`
...
- `saved_af_email_state.json` — `{schedule_id: [ref, ...]}`, every ref ever actually emailed for that Auto Fetch schedule (cumulative, not just last cycle); each cycle drops already-sent refs before emailing and skips entirely if nothing new is left (the Telegram summary still sends every cycle regardless, unfiltered) — cleared for a schedule when it's removed via 🗑 Remove Schedule
