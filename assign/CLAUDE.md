...
- **`dlv_batch.py`**, **`dlv_tasks.py`**, **`morning_briefing.py`**, **`fetch_tasks.py`**, **`refresh_auth.py`**, **`sectional_properties.py`**, **`auto_fetch.py`**, **`receive_tasks.py`**, **`lookup_reference.py`**, **`valuer_tasks.py`**, **`job_distribution.py`**, **`new_assignment.py`** — one feature each, extracted out of `bot.py`. `lookup_reference.py` exports `_lu_search_ref`/`_lu_fetch_detail`/`_lu_format_result` as a public API — `new_assignment.py`'s `_lookup_one_ref`/`_post_assignment_report` import these directly rather than duplicating the search/detail/format logic (these two functions moved out of `bot.py` along with the rest of New Assignment; they were never Lookup Reference's to own). `valuer_tasks.py` and `job_distribution.py` each keep their own near-identical `_vt_fetch_task_detail`/`_jd_fetch_task_detail` (token-rotation + retry against the shared detail-view endpoint) rather than one importing the other's — kept as independent copies (see "Optional cleanup pass" below for the deferred promotion). `job_distribution.py` also exports `_JD_STATUS` — `bot.py`'s `cmd_export_status` (a thin combiner also reading Bulk Export's still-inline `_BE_STATUS`) imports it rather than owning the dict itself. `new_assignment.py` also owns `cmd_assignments` (the `/assignments` history viewer, `BTN_ASSIGNMENTS`) — it was never part of "saved-valuers management" and always belonged with New Assignment.
...
| ~~Job Distribution~~ | ~~`JD`~~ | ~~~633 lines~~ | **done — `job_distribution.py`** |
| ~~New Assignment (+ /assignments)~~ | ~~`S`~~ | ~~~840 lines~~ | **done — `new_assignment.py`** |
| Bulk Export | `BE` | ~1,000+ lines | `BESession` dataclass sits far from its own logic/handlers, but everything now physically between them is permanent bot.py content (`cmd_start`/`cmd_valuers`/`cmd_daemon`/etc., or pointer comments for already-extracted features) — no other feature blocks this extraction anymore |
...
2. `cmd_export_status` (still in `bot.py`) renders both Bulk Export's `_BE_STATUS` (still inline) and Job Distribution's `_JD_STATUS` (now imported from `job_distribution.py`) in one message — owned by neither; stays a thin combiner in `bot.py` until Bulk Export also extracts (step 10), at which point it can move wherever makes sense or get resolved for real.
3. ~~New Assignment's `recv_confirm` calls `_post_assignment_report`/`_lookup_one_ref`, built on Lookup Reference's primitives~~ — resolved: `lookup_reference.py` exports `_lu_search_ref`/`_lu_fetch_detail`/`_lu_format_result`; `_post_assignment_report`/`_lookup_one_ref` moved into `new_assignment.py` along with the rest of New Assignment (step 9), importing those three primitives directly from `lookup_reference.py`.
...
6. ~~Valuer Tasks' `_vt_run` called Job Distribution's `_jd_fetch_task_detail`/`_jd_headers`/`_JD_WORKERS` (undocumented until Valuer Tasks was extracted)~~ — resolved: `valuer_tasks.py` got its own `_vt_fetch_task_detail`/`_vt_headers`/`_VT_WORKERS` (same logic, same detail-view endpoint, its own retry constants) instead of reaching into `bot.py`'s Job Distribution section. Now that Job Distribution is also extracted (`job_distribution.py`, with its own equally-independent `_jd_fetch_task_detail`/`_jd_headers`/`_JD_WORKERS`), the two copies are confirmed still identical in shape — promoting the pattern into `token_rotator.py` is still deferred to the optional cleanup pass, not done preemptively.
...
8. ~~**Job Distribution**~~ — done (`job_distribution.py`). Same pattern as Valuer Tasks — own `_jd_fetch_task_detail`/`_jd_headers`/`_JD_WORKERS` copy. Left `cmd_export_status` in `bot.py` for now (thin combiner reading both BE's still-inline `_BE_STATUS` and JD's now-imported `_JD_STATUS`) until Bulk Export also lands.
9. ~~**New Assignment**~~ — done (`new_assignment.py`). The original core flow, importing Lookup Reference's primitives directly for `_lookup_one_ref`/`_post_assignment_report`. Also picked up `cmd_assignments` (the `/assignments` history viewer) since it was never truly "saved-valuers management" and belongs with New Assignment. **Did not** migrate `cmd_assignments`'s manual chunking loop onto `telegram_report.py` — left as-is (moved verbatim) to keep this step a pure extraction; that migration is still deferred to the optional cleanup pass below, alongside the other truncate/chunk-pattern spots.
10. **Bulk Export** — last remaining feature. `_TokenRotator`/`_be_cred_keyboard` already live in shared modules and no other feature's code sits between `BESession` and its logic/handlers anymore (see contiguity note above), so this is mostly mechanical extraction. Resolve `cmd_export_status` for real (import both modules' status dicts, or move it wherever makes sense once both own their data).

**Optional cleanup pass** (anytime after step 10): a shared `excel_report.py` for the openpyxl styling boilerplate duplicated across `_dt_build_excel` (already extracted), `_be_build_excel`, `_jd_build_excel`, `_vt_build_excel` (identical bold-blue-header/autofilter/frozen-pane/auto-width pattern in all four) — plus sweeping the three remaining single-message truncate-only spots (`recv_confirm` in `new_assignment.py`, `_do_assign_tasks` in `receive_tasks.py`, `_rt_fetch_and_show` in `receive_tasks.py`) onto a lightweight truncate helper, distinct from `telegram_report.py`'s multi-chunk `_send_chunked_report` since these only ever send one message. Also migrate `new_assignment.py`'s `cmd_assignments` off its manual chunking loop onto `telegram_report._send_chunked_report` (deferred from step 9 to keep that step a pure extraction). Also consider promoting `valuer_tasks.py`'s `_vt_fetch_task_detail` and `job_distribution.py`'s `_jd_fetch_task_detail` (confirmed identical in shape — both wrap a detail-view GET in the same token-rotation-with-retry loop) into a single shared helper in `token_rotator.py` — deferred twice already (steps 7 and 8) specifically to avoid a premature abstraction before both real call sites existed side by side.
...
- Built on `python-telegram-bot` v21; owns `BE` (Bulk Export, not yet extracted) directly, plus `cmd_export_status` (a thin combiner over BE's `_BE_STATUS` and Job Distribution's imported `_JD_STATUS`) and the permanent orchestrator functions (`cmd_start`/`cmd_help`, saved-valuers management, daemon control, token/error-report status, `cmd_restart`) — every other feature (`S`/New Assignment, `RS`/Receive Tasks, `LU`/Lookup Reference, `VT`/Valuer Tasks, `JD`/Job Distribution, plus all the earlier extractions) now lives in its own sibling module, each imported and registered via its own `register(app)`
