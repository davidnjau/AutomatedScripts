...
6. Valuer Tasks' `_vt_run` turned out to call Job Distribution's `_jd_fetch_task_detail`/`_jd_headers`/`_JD_WORKERS` — an undocumented dependency only discovered once Valuer Tasks was being extracted (Job Distribution hadn't extracted yet). Resolved by giving `valuer_tasks.py` its own `_vt_fetch_task_detail`/`_vt_headers`/`_VT_WORKERS` copy rather than reaching into `bot.py`'s still-inline Job Distribution section. When Job Distribution extracted next, it got the equivalent independent copy — the two are confirmed identical in shape; promoting the pattern into `token_rotator.py` was deliberately deferred (see "Optional cleanup pass" below) rather than done preemptively before both real call sites existed side by side.

**Notes specific to the last few steps:**
- **New Assignment** also picked up `cmd_assignments` (the `/assignments` history viewer) since it was never truly "saved-valuers management" — it always belonged with New Assignment. Its manual chunking loop was *not* migrated onto `telegram_report.py` during that extraction, to keep the step a pure move; that migration is deferred to the optional cleanup pass below.
...
**Optional cleanup pass** (not yet done, no blocker to starting it): a shared `excel_report.py` for the openpyxl styling boilerplate duplicated across `_dt_build_excel`, `_be_build_excel`, `_jd_build_excel`, `_vt_build_excel` (identical bold-blue-header/autofilter/frozen-pane/auto-width pattern in all four) — plus sweeping the three remaining single-message truncate-only spots (`recv_confirm` in `new_assignment.py`, `_do_assign_tasks` in `receive_tasks.py`, `_rt_fetch_and_show` in `receive_tasks.py`) onto a lightweight truncate helper, distinct from `telegram_report.py`'s multi-chunk `_send_chunked_report` since these only ever send one message. Also migrate `new_assignment.py`'s `cmd_assignments` off its manual chunking loop onto `telegram_report._send_chunked_report`. Also consider promoting `valuer_tasks.py`'s `_vt_fetch_task_detail` and `job_distribution.py`'s `_jd_fetch_task_detail` (confirmed identical in shape — both wrap a detail-view GET in the same token-rotation-with-retry loop) into a single shared helper in `token_rotator.py`.
