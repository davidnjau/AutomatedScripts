...
- `saved_incremental_notify_config.json` — `{enabled, interval_minutes, email}` for 🔢 Incremental's "🔔 Notify on Fill" single repeating job (one config, not per-schedule like Auto Fetch); set via that menu action, which also (re)schedules the job (`ic_notify_job`) immediately and on bot startup if `enabled`
- `saved_incremental_notify_state.json` — `{closed_batches, reported_cleared_refs}`, what the last Notify on Fill cycle already knew about, so a batch/cleared-ref already reported never repeats — closed batches are re-derived from `saved_incremental_closed_batches.json` each cycle, `reported_cleared_refs` accumulates ref-by-ref as new clearances are reported
