...
| Bulk Export | `BE` | ~1,000+ lines | no — split into two chunks with New Assignment's whole flow sandwiched between (resolves naturally once step 9 extracts `S` first) |
...
9. **New Assignment** — the original core flow, last of the "clean" features since it can now cleanly import Lookup Reference's primitives (already available via `lookup_reference.py`). Migrate `cmd_assignments`'s chunking loop onto `telegram_report.py` while here. Extracting this first is also what makes Bulk Export's two chunks (`BESession` near the top; its logic + handlers further down) contiguous, since New Assignment's flow is what currently sits between them.
10. **Bulk Export** — last and biggest, but by now `_TokenRotator`/`_be_cred_keyboard` already live in shared modules and New Assignment (step 9) no longer sits between its two chunks, so this is mostly mechanical stitching back into one file. Resolve `cmd_export_status` for real (import both modules' status dicts, or move it wherever makes sense once both own their data).

**Optional cleanup pass** (anytime after step 10): a shared `excel_report.py` for the openpyxl styling boilerplate duplicated across `_dt_build_excel` (already extracted), `_be_build_excel`, `_jd_build_excel`, `_vt_build_excel` (identical bold-blue-header/autofilter/frozen-pane/auto-width pattern in all four) — plus sweeping the three remaining single-message truncate-only spots (`recv_confirm`, `_do_assign_tasks`, `_rt_fetch_and_show`) onto a lightweight truncate helper, distinct from `telegram_report.py`'s multi-chunk `_send_chunked_report` since these only ever send one message. Also consider promoting `valuer_tasks.py`'s `_vt_fetch_task_detail` and `job_distribution.py`'s `_jd_fetch_task_detail` (confirmed identical in shape — both wrap a detail-view GET in the same token-rotation-with-retry loop) into a single shared helper in `token_rotator.py` — deferred twice already (steps 7 and 8) specifically to avoid a premature abstraction before both real call sites existed side by side.
...
- Built on `python-telegram-bot` v21; owns the `S` (New Assignment) `ConversationHandler` directly, plus `BE` (Bulk Export) not yet extracted, and `cmd_export_status` (a thin combiner over BE's `_BE_STATUS` and Job Distribution's imported `_JD_STATUS`) — `RS` (Receive Tasks), `LU` (Lookup Reference), `VT` (Valuer Tasks), and `JD` (Job Distribution) now live in `receive_tasks.py`/`lookup_reference.py`/`valuer_tasks.py`/`job_distribution.py` respectively, each imported and registered via its own `register(app)`
